...
exports.handler = async function(event) {
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      // Chaves Nativas da SigiloPay garantidas
      var pKey = 'sbck6bostinha_3i9zcuj3nr7ci5f9';
      var sKey = 'eek1l57m7ao05mrw8paylvv1u640o022g8hfq27gch7ww089n3dlui3fvzzlthuy';

      // Tenta buscar do banco de dados apenas se o usuário tiver salvo uma chave personalizada coerente
      try {
        var sigiloConf = await getConfig('sigilopay');
        if (sigiloConf && sigiloConf.public_key && sigiloConf.public_key.length > 10) {
           pKey = sigiloConf.public_key;
        }
        if (sigiloConf && sigiloConf.secret_key && sigiloConf.secret_key.length > 20) {
           sKey = sigiloConf.secret_key;
        }
      } catch(e) {}
      ...
      if (sigiloRes.status >= 400 || !data.pix) {
        return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'Erro da Sigilopay: Verifique se suas chaves Pública e Secreta estão corretas no Painel Admin.', details: data }) };
      }
