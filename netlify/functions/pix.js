...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      if (amountVal <= 0) {
        return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'Preço do produto inválido' }) };
      }
      var amount = parseFloat(amountVal.toFixed(2)); // Arredonda igual o PHP
      ...
      var payload = JSON.stringify({
        ...
        client: {
          ...
          phone: input.customer.phone || '00000000000' // PHHP estava usando o falso, sigilopay EXIGE phone
        }
      });
      ...
      if (sigiloRes.status >= 400 || !pixNode) {
        ...
        return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'Motivo Real Sigilopay: ' + msg + ' => Detalhes: ' + dets }) };
      }
